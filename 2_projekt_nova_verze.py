...
def again_guess(guess_number,guess_numbers) -> str:
    """
    Description:
    Check function to compare guess number with list guess numbers, where are previously tips.
    If user guess the same number, will get information about it.
    """
    if guess_number in guess_numbers:
        print("You've already guessed this number")

def wrong_value() -> str:
    """
    Description:
    Information for users about wrong enter number.
    """
    print('''Wrong value. You don't give 4 numeric characters or
...
def ending() -> str:
    """
    Description:
    Information for users about win the game and numbers their tips to win.
    """
...
def game_function():
    ...
    while game_cont:
        ...
        again_guess(user_tip,user_tips)
    
        if user_tip.isnumeric() and user_tip[0] != '0' and len(user_tip) == 4:
